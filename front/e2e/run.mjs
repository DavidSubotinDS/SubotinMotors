...
const payment = resolve(front, '../services/payment-service');
const windows = process.platform === 'win32';
const children = new Set();
// Do not inherit developer Spring/Stripe/mail/Vite configuration or .env files.
const env = Object.fromEntries(Object.entries(process.env).filter(([key]) =>
  !/^(E2E_|IDENTITY_|NOTIFICATION_|PAYMENT_|RABBITMQ_|MYSQL_|DB_|PUBLIC_URL$|SESSION_|SPRING_|STRIPE_|APP_|SMTP_|PAYMENTS_|AUCTION_|VITE_|GATEWAY_|SERVER_|MANAGEMENT_|LOGGING_|JAVA_TOOL_OPTIONS$|JDK_JAVA_OPTIONS$|_JAVA_OPTIONS$)/i.test(key)));
...
try {
  ...
  env.PAYMENT_URL = 'http://127.0.0.1:18084';
  env.GATEWAY_PAYMENT_URL = env.PAYMENT_URL;
  env.E2E_PAYMENT_CONTROL_URL = env.PAYMENT_URL;
  env.PAYMENT_PORT = '18084';
  console.log('Compiling test-only identity/backend launchers and building isolated frontend...');
  const mavenArgs = ['--batch-mode', '--no-transfer-progress', 'clean', 'test-compile', 'dependency:build-classpath',
  ...
  if (windows) await run('cmd.exe', ['/d', '/s', '/c', 'mvnw.cmd', ...mavenArgs], payment, 'build-payment.log');
  else await run('bash', ['./mvnw', ...mavenArgs], payment, 'build-payment.log');
  ...
  const paymentClasspath = [resolve(payment, 'target/test-classes'), resolve(payment, 'target/classes'),
    (await readFile(resolve(payment, 'target/e2e-classpath.txt'), 'utf8')).trim()].join(delimiter);
  ...
  const paymentServer = start(java, ['-Duser.timezone=UTC', '-cp', paymentClasspath,
    'lithan.autostrada.payment.PaymentE2eApplication'], payment, 'payment.log');
  await ready(paymentServer, 'http://127.0.0.1:18084/__e2e/ready', true);
  ...
  const edge = start(java, ['-jar', resolve(gateway, 'target/gateway-0.0.1-SNAPSHOT.jar'),
    ...
    '--gateway.payment-url=http://127.0.0.1:18084',
