...
| Reactive communication | Gateway uses WebFlux and an asynchronous WebClient identity exchange. The next observability stage must demonstrate actual nonblocking service communication, bounded timeout/failure behavior and trace correlation. S9 checkout fan-out is deferred; do not claim it exists. Acceptance evidence for the chosen reactive flow is still outstanding. |
...
| CI on push and PR, build/test/artifacts | Backend/Frontend cover both events, all four application builds plus gateway, tests, artifacts, containers and broker/MySQL/browser gates. This branch adds PMD/ESLint and Windows credential-script checks to those required gates. |
...
| Static analysis integrated into CI | Implemented locally on the static-analysis branch: PMD across all Java owners/gateway and ESLint across frontend/tooling; failures block Backend/Frontend and reports are uploaded. See [rules, scope and evidence](static-analysis.md). Branch remote CI remains an owner gate. |
...
architecture, not unfinished mandatory extraction for this delivery. The next
implementation is S12 observability for the four current application owners and
gateway, including evidence of reactive communication. Then assemble S13 defence
evidence. Local CD already exists. Public hosting, registry publication and gRPC
